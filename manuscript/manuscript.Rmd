...
Differences between Eocene and modern temperatures at a certain latitude were calculated by randomly pairing all iterations of the posterior from the Eocene and modern temperature gradient model, calculating the Eocene and modern temperature using the respective iterations, taking the difference, and then calculating the median (2.5th, 97.5th percentile) from all pairs of iterations.  

Global average temperatures were calculated by taking the weighted mean of the median (2.5th, 97.5th percentile) temperature estimates in 1 degree latitudinal bins. The weights were set to the proportion of global surface area in each latitudinal bin, i.e. decreasing with increasing latitude as:
...
```{r results block 1 - for in-line code, echo = FALSE}
