...
We estimated the posterior distributions of the model parameters using a custom-made Markov chain Monte Carlo (MCMC) algorithm, written in R. Specifically, we sampled the unknown parameters $A$, $K$, $M$ and $B$ using a Metropolis-Hastings step, and used Gibbs sampling to estimate all other unknown parameters. Posterior inference on the modern gradient is based on four chains with 60,000 iterations each, 10,000 of which were discarded as burn-in, and keeping every 10th iteration, resulting in a total of 20,000 iterations. For the Eocene model, we increased these numbers by a factor of 10, i.e. running four chains with 600,000 iterations each, keeping every 100th iteration, as the hierarchical model structure results in higher autocorrelation of the chains.  
