...
proxy_temp <- temp_from_gradient(lat = lat, model_out = mode_all_prox)
...
par(mar=c(4.2,4.2,0.5,0.5), mgp = c(2.5,0.8,0)
    ,las = 1)
plot(obsmat$p_lat,obsmat$temperature, pch = pch1[proxind1], col = col1[proxind1], xlim = c(-2,92), ylim = c(0,42.5),
     xlab = xlab1, ylab = ylab1, cex = cex1[proxind1], type = "n", xaxs = "i", yaxs = "i")



error_polygon(lat, all_temp$l_ci_95, all_temp$u_ci_95, col = rgb(0,0.3,0.8,0.15))
error_polygon(lat, proxy_temp$l_ci_95, proxy_temp$u_ci_95, col = col_proxgrad_shade)


points(lat, all_temp$median, type = "l", lwd = 2, col = rgb(0,0.3,0.8,0.5))
points(lat, proxy_temp$median, type = "l", lwd = 2, col = col_proxgrad)
