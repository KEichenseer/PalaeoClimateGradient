...
png("figures/SM/S1_traceplot_eeco_yestimate.png", width = 4.5, height = 5, units = "in", res = 400)
plot_chains(eeco_mod, select = list("yestimate", c(1,8,25,30)), xlabel = expression("iteration"), ylabel = unlist(sapply(c(1,8,25,30), function(x) 
  bquote(mu[.(x)]))),
            xaxt = "n", xaxsat = c(1,c(1,2,3,4,5,6)*10^5), xaxslab = c(1,"100,000","200,000","300,000",
                                                                       "400,000","500,000","600,000"))
dev.off()

## comment: these are oddly large in some cases
# 3. Traceplot of some of the location sd
png("figures/SM/S1_traceplot_eeco_yestimate.png", width = 4.5, height = 5, units = "in", res = 400)
plot_chains(eeco_mod, select = list("sdyest", c(1,8,12,22)), xlabel = expression("iteration"), ylabel = unlist(sapply(c(1,8,25,30), function(x) 
  bquote(mu[.(x)]))),
  xaxt = "n", xaxsat = c(1,c(1,2,3,4,5,6)*10^5), xaxslab = c(1,"100,000","200,000","300,000",
                                                             "400,000","500,000","600,000"))
dev.off()