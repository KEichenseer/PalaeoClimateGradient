...
location_sample_size <- as.numeric(c(table(obsmat$sample), c(rep(1,11))))
...
generate_lat_samples <- function(n) sample(eocene_lat,n,replace = F)
...
gradient_formulas <- rev(list( function(x) 30-x^2/1080 + rnorm(length(x), 0, 4.39), # extreme greenhouse
                               ...
                               function(x) gradient(x, c(-1.775, 30.115, 42.070,  0.106)) + rnorm(length(x), 0, 4.39), # now use modern gradient instead # 11.25*atan(-0.1*x+3.5)+15.25, 
...
sample_sizes <- c(5,10,20,34)


# eocene_sample_nIter <- 250000 #30000 # 250000
random_sample_nIter <- 40000 #4000 # 10000
n_adapt_sd_random <- 5000 
# n_adapt_sd_eocene <- 5000 
n_thin_random <- 100 #25

burnin_random_sample <- 10000 # 1000 # 5000
# burnin_eocene_sample <- 50000 # 10000 # 50000
...
for(ss in 1:length(sample_sizes)) {
  m_1[[ss]] <- list()
  for(rep in 1:n_rep) {
    lat <- generate_lat_samples(sample_sizes[ss]) 
    temp <- gradient_formulas_static[[grad_type]](lat) + rnorm(sample_sizes[ss],0,modern_sd)
      
    # construct obsmat
    loc_ss <- sample(location_sample_size, sample_sizes[ss])
    
    obslat <- rep(lat, loc_ss)
    obssample <- rep(seq_len(sample_sizes[ss]), loc_ss)
    obstemp <- rep(temp, loc_ss) + rnorm(sum(loc_ss),0,observation_sd)

    obsmat_s <- data.frame(
      sample = obssample,
      p_lat = obslat, 
      temperature = obstemp,
      sd = rep(NA,sum(loc_ss)))
    
    m_1[[ss]][[rep]] <- hierarchical_model(n_iter = random_sample_nIter,
                          n_thin = n_thin_random,
                          obsmat = obsmat_s,
                          distrmat = NULL,
                          prior_input = priors,
                          adapt_sd = n_adapt_sd_random)
...
par(mfrow=c(5,4), mar = c(4,4,.5,.5))
for(i in 1:4) {
  plot(0,0,type = "n", xlim = c(0,90), ylim = c(0,40), xlab = "abs. latitude", ylab = "temperature")
  points(lats, gradient_formulas_static[[grad_type]](lats), type = "l", lwd = 2)
...
for(ss in 1:length(sample_sizes)) {
  m_2[[ss]] <- list()
  for(rep in 1:n_rep) {
    lat <- floor(generate_lat_samples(sample_sizes[ss]))+0.5 
    ...
    m_2[[ss]][[rep]] <- hierarchical_model(n_iter = random_sample_nIter,
                                           n_thin = n_thin_random,
                                           obsmat = obsmat_s,
                                           distrmat = NULL,
                                           prior_input = priors,
...
for(ss in 1:length(sample_sizes)) {
  ...
  for(rep in 1:n_rep) {
    ...
    m_3[[ss]][[rep]] <- hierarchical_model(n_iter = random_sample_nIter,
                                           n_thin = n_thin_random,
                                           obsmat = obsmat_s,
                                           distrmat = NULL,
                                           prior_input = priors,
...
for(ss in 1:length(sample_sizes)) {
  ...
  for(rep in 1:n_rep) {
    ...
    m_4[[ss]][[rep]] <- hierarchical_model(n_iter = random_sample_nIter,
                                           n_thin = n_thin_random,
                                           obsmat = obsmat_s,
                                           distrmat = NULL,
                                           prior_input = priors,
...
cols = rev(c("green","red", "orange", "blue"))
for(i in 1:4) points(lats,gradient_formulas_static2[[i]](lats),type = "l", col = cols[i], lwd = 2)
### get temperatures from combining all gradients
t_list <- list()
for(i in 1:4) { # this can take a few minutes
  t_list[[i]] <- list()
  for(j in 1:4) {
    ...
    t_list[[i]][[j]] <- data.frame(lat = lats,
                                   ...
                                   known = gradient_formulas_static[[i]](lats)
                                   
    )
  }
  names(t_list[[i]]) <- c(5,10,20,"eocene")
}
names(t_list) <- c("extreme_icehouse","icehouse (modern)","greenhouse","extreme_greenhouse")
