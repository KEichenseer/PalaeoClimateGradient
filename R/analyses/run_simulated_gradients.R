...
m_1[[5]][[1]] <-  run_MCMC_simple(n_iter = 250000,
                                  n_thin = 20,
                                  x = eocene_lat,
...
m_1_proc <- list()
for(i in 1:length(sample_sizes)) m_1_proc[[i]] <- combine_posterior(m_1[[i]],burnin = 5000)
m_1_proc[[5]] <- combine_posterior(m_1[[5]],burnin = 50000)
dim(m_1_proc[[5]])
...
m_2[[5]][[1]] <-  run_MCMC_simple(n_iter = 250000,
                                  n_thin = 20,
                                  x = eocene_lat,
                                  y = gradient_formulas[[grad_type]](eocene_lat),
                                  prior_input = priors,
...
m_2_proc <- list()
for(i in 1:length(sample_sizes)) m_2_proc[[i]] <- combine_posterior(m_2[[i]],burnin = 5000)
m_2_proc[[5]] <- combine_posterior(m_2[[5]],burnin = 50000)
dim(m_2_proc[[5]])
...
m_3[[5]][[1]] <-  run_MCMC_simple(n_iter = 250000,
                                  n_thin = 20,
                                  x = eocene_lat,
                                  y = gradient_formulas[[grad_type]](eocene_lat),
                                  prior_input = priors,
...
m_3_proc <- list()
for(i in 1:length(sample_sizes)) m_3_proc[[i]] <- combine_posterior(m_3[[i]],burnin = 5000)
m_3_proc[[5]] <- combine_posterior(m_3[[5]],burnin = 50000)
dim(m_3_proc[[5]])
...
m_4[[5]][[1]] <-  run_MCMC_simple(n_iter = 250000,
                                  n_thin = 20,
                                  x = eocene_lat,
                                  y = gradient_formulas[[grad_type]](eocene_lat),
                                  prior_input = priors,
...
m_4_proc <- list()
for(i in 1:length(sample_sizes)) m_4_proc[[i]] <- combine_posterior(m_4[[i]],burnin = 5000)
m_4_proc[[5]] <- combine_posterior(m_4[[5]],burnin = 50000)
dim(m_4_proc[[5]])
...
for(ss in 1:length(sample_sizes)) {
  m_5[[ss]] <- list()
  for(rep in 1:n_rep) {
    lat <- floor(180/pi * asin(runif(sample_sizes[ss],0,1)))+0.5
...
m_5[[5]][[1]] <-  run_MCMC_simple(n_iter = 250000,
                                  n_thin = 20,
                                  x = eocene_lat,
...
m_5_proc <- list()
for(i in 1:length(sample_sizes)) m_5_proc[[i]] <- combine_posterior(m_5[[i]],burnin = 5000)
m_5_proc[[5]] <- combine_posterior(m_5[[5]],burnin = 50000)
dim(m_5_proc[[5]])
...
for(i in 1:5) {
  ...
  points(lats,temp_5[[i]][1,], type = "l", lwd = 2, col = rgb(1,0,0,0.75))
}
...
for(i in 1:5) points(lats,gradient_formulas[[i]](lats),type = "l", col = cols[i], lwd = 2)
