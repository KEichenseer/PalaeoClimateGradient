...
random_sample_nIter <- 10000 #4000 # 10000
n_adapt_sd_random <- 5000 
# n_adapt_sd_eocene <- 5000 
n_thin_random <- 25 #25

burnin_random_sample <- 5000 # 1000 # 5000
...
for(ss in 1:length(sample_sizes)) {
  ...
  for(rep in 1:n_rep) {
    ...
    if(sample_sizes[ss]==34) lat <- eocene_lat
    temp <- gradient_formulas_static[[grad_type]](lat) + rnorm(sample_sizes[ss],0,observation_sd)
      
  
    m_1[[ss]][[rep]] <- run_MCMC_simple(
                          x = lat,
                          y = temp,
                          n_iter = random_sample_nIter,
...
saveRDS(four_gradients_model, "D://OneDrive - Durham University/projects/four_gradients_revised_model_proc_M_wider_noise.rds")
saveRDS(four_gradients_temp, "D://OneDrive - Durham University/projects/four_gradients_revised_temp_M_wider_noise.rds")
...
saveRDS(t_long,"./results/simulation/simulated_gradients_temperature_estimates_with_noise_eocene_resampled_M_wider_noise.rds")
